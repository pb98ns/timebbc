...
@section('content')
    ...
    <div class="row justify-content-center">
                ...
                <center>
            <h3>    <div class="card-header">{{ __(' KPiR - deklaracje zaplanowane i zgłoszone do realizacji') }} </div> </h3>
                 </center>
  ...
  <tbody>
    ...
    <tr>
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}" >{{ $loop->iteration }}. {{$projects->firm->name}} @if(!empty($projects->firm->number)) ({{$projects->firm->number}}) @endif</td> 
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}"> {{$projects->vat}} {{$projects->pit5_cit}} {{$projects->vat_ue}} {{$projects->vat_uea}} {{$projects->vat_uec}} ({{$projects->vat_27}}) {{$projects->akc}} {{$projects->cit_st}} {{$projects->amortyzacja}} @if(!empty($projects->korekta)) <b>(korekta)</b> @endif</td> 
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}">{{$projects->user->name}} {{$projects->user->surname}}</td> 
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}">{{$projects->close_date}} {{$projects->close_time}}</td>
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}"><b>{{$projects->status1}}<b></td> 
<td class="table-danger table-row" data-href="{{action('MonthController@showkpir', $projects->czas) }}">@if(!empty($projects->uwagi || $projects->uwagidokorekty))TAK @else NIE  @endif</td> 


    <td class="table-danger">
      <form method = "post" class="delete_from2" action="{{action('MonthController@delete',$projects->czas )}}" title="Usuń">
      {{csrf_field()}}
...
<div class="row justify-content-center">
    <div class="form-row">
     <form action="{{action('MonthController@searchkpir')}}" method="POST" role="form">
     @csrf
...
<div class="text-center">
 ...
 <br>
                ...
                <center>
            <h3>    <div class="card-header">{{ __(' KPiR - deklaracje zrealizowane w okresie od') }} {{$start_date}} do {{$end_date}}</div> </h3>
                 </center>
...
<td class="table-success table-row" data-href="{{action('MonthController@show', $projects->czas) }}"><a href="{{URL::to('/home/month_end_closing/update_kpir_status/'.$projects->czas)}}" class="btn btn-primary a-btn-slide-text" title="Cofnij potwierdzenie">
...
<div class="container">
    <div class="row justify-content-center">
                ...
                <center>
            <h3>    <div class="card-header">{{ __(' KPiR - deklaracje niezrealizowane w okresie od') }} {{$start_date}} do {{$end_date}}</div> </h3>
                 </center>
